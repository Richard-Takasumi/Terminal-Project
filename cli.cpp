    ...
    */
    ...
    else if (strcmp(cmd, "rm") == 0)
    {
        ...
        */
        ...
        if (strcmp(flag, "-r") == 0) {
            deleteFile(wd, name);
            deleteDir(wd, name, true);
        } else {
            deleteFile(wd, flag);
            deleteDir(wd, flag, false);
        }
    ...
    else if (strcmp(cmd, "mv") == 0)
    {
        ...
        */
        ...
        fetch(rest, target);
        char destination[MAX_CMD_LEN] = "";
        fetch(rest, destination);        

        //target is either a file or a dir, but only know name of it.
        //search wd subfiles and subdirs until found 
        bool target_is_file = false;
        bool target_is_dir = false;
        Dir* p_target_dir = nullptr;
        File* p_target_file = nullptr;

        // search current wd for a file w/ name that matches the target name
        if (wd->subfile != nullptr) {
            for (File* p = wd->subfile; p!=nullptr; p=p->next) {
        ...
        // search the current wd for a directory w/ name that matches the target name
        if (wd->subdir != nullptr) {
            for (Dir* p = wd->subdir; p!=nullptr; p=p->next) {
                if (strcmp(p->name, target) == 0) {
                    target_is_dir = true;
                    p_target_dir = p;
                }
        ...
        Dir* p_destination_dir = nullptr;

        // search for pointer of destination (which must be a directory);
        if (wd->subdir != nullptr) {
            for (Dir* p_two = wd->subdir; p_two!= nullptr; p_two=p_two->next) {
                if (strcmp(p_two->name, destination) == 0 ){
                    p_destination_dir = p_two;
                    if (target_is_dir && destination != nullptr) {
                        moveDir(p_target_dir, p_destination_dir);
                    } else if (target_is_file && destination != nullptr) {
                        moveFile(p_target_file, p_destination_dir);
                    }
                    break;
                }
            }
