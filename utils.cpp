...
bool valSystem(const Dir *, const File *);

bool valSystem(const Dir *, const Dir *);

bool valSystem(const Dir*);

void testCase( bool status, int &caseNumber) {
    if (status == true ) {
        cout << "test case "<< caseNumber <<" --> Passed" << endl;
    } else {
        cout << "test case "<< caseNumber <<" --> Failed" << endl;
    }
    caseNumber++;
}
...
Dir *sampleSystem()
{
    ...
    videos->parent = download;
    videos->subdir = NULL;
    videos->subfile = NULL;
    videos->next = nullptr;
    download->subdir = videos;
    // /memo.txt
    File *memo = new File();
    ...
    // /comp2011/Bing
    Dir *Bing = new Dir();
    strcpy(Bing->name, "Bing");
    Bing->parent = comp2011;
    Bing->subdir = NULL;
    Bing->subfile = NULL;
    Bing->next = NULL;
    pa3->next = Bing;
    // /comp2011/BingChilling
    Dir *BingChilling = new Dir();
    strcpy(BingChilling->name, "BingChilling");
    BingChilling->parent = comp2011;
    BingChilling->subdir = NULL;
    BingChilling->subfile = NULL;
    BingChilling->next = NULL;
    Bing->next = BingChilling;
    ...
    pa3->subfile = about;
    // /comp2011/Bing/aboutBingChilling.pdf
    File *aboutBingChilling = new File();
    strcpy(aboutBingChilling->name, "aboutBingChilling");
    aboutBingChilling->size = 60;
    aboutBingChilling->tag = DOC;
    aboutBingChilling->parent = BingChilling;
    aboutBingChilling->next = nullptr;
    BingChilling->subfile = aboutBingChilling;
    // /comp2011/Bing/aboutBingChilling.pdf
    File *sauce = new File();
    strcpy(sauce->name, "sauce.mp4");
    sauce->size = 60;
    sauce->tag = VIDEO;
    sauce->parent = BingChilling;
    sauce->next = nullptr;
    aboutBingChilling->next = sauce;
    ...
    x = createFile(Bing, nullptr, 64, VIDEO);
    testCase(x==1, caseCount);
    x = createFile(comp2011, nullptr, 64, VIDEO);
    testCase(x==1, caseCount);
    x = createFile(pa3, nullptr, 64, VIDEO);
    testCase(x==1, caseCount);
    x = createFile(nullptr, nullptr, 64, VIDEO);
    testCase(x==1, caseCount);
    x = createFile(nullptr, " ", 64, VIDEO);
    testCase(x==1, caseCount);
    x = createFile(nullptr, ".", 64, VIDEO);
    testCase(x==1, caseCount);
    x = createFile(nullptr, "..", 64, VIDEO);
    ...
    x = createFile(Bing, " ", 32, OTHER);
    testCase(x==2, caseCount);
    x = createFile(Bing, ".", 32, OTHER);
    testCase(x==2, caseCount);
    x = createFile(Bing, "..", 32, OTHER);
    testCase(x==2, caseCount);
    ...
    x = createFile(Bing, ".-.", 32, VIDEO);
    testCase(x==0, caseCount);
    x = createFile(Bing, ".-.", 0, VIDEO);
    testCase(x==3, caseCount);
    
    x = createDir(Bing, "fd");
    testCase(x==0, caseCount);
    x = createDir(Bing, "fd2");
    testCase(x==0, caseCount);
    x = createFile(Bing, "fd", 0, VIDEO);
    testCase(x==3, caseCount);
    ...
    x = moveDir(Bing, comp2011);
    testCase(x == 2, caseCount);
    x = moveDir(Bing, download);
    testCase(x == 0, caseCount);
    x = moveDir(Bing, Bing);
    testCase(x == 4, caseCount);
    
    x = getSize(download);
    testCase(x == 809, caseCount);

    int comp2011_size = main->size+bin->size+about->size+aboutBingChilling->size+sauce->size;
    x = getSize(comp2011);
    testCase(x == comp2011_size, caseCount);

    int pa3_size = main->size+bin->size+about->size;
    x = getSize(pa3);
    testCase(x == pa3_size, caseCount);
